...
calculateMetrics(){
    ...
    # Prepare results to be plotted by gnuplot. prefix each value with "1 param: "
    # I can't seem to place the line number from the first command after 'param: "1<"' as well..
    nl -w2 -s ' param: ' "$WORKING_DIR/result.dat" >> "$WORKING_DIR/result_F.dat" && sed -i -e 's/^ //g' "$WORKING_DIR/result_F.dat"
}
